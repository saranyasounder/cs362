...
// Add reactive store using existing functions
class ReactiveCurrentChart {
  private data = $state(loadCurrentChartData());

  get chart() {
    return this.data;
  }

  update(updates: Partial<Chart>) {
    this.data = { ...this.data, ...updates };
    updateCurrentChartData(this.data); // Uses your existing function
  }

  load(chart: Chart) {
    this.data = { ...chart };
    updateCurrentChartData(this.data);
  }
}

export const currentChart = new ReactiveCurrentChart();